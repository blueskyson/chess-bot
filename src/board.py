...
class Board:
    ...
    def user_perform_move(self, move, status):
        ...
        if status == PROMOTION:
            ...
            if move.promotion == 'b':
                self.chesspieces[move.xto][move.yto] = pieces.Bishop(move.xto, move.yto, piece.color)
            elif move.promotion == 'n':
                self.chesspieces[move.xto][move.yto] = pieces.Knight(move.xto, move.yto, piece.color)
            elif move.promotion == 'r':
                self.chesspieces[move.xto][move.yto] = pieces.Rook(move.xto, move.yto, piece.color)
            self.chesspieces[move.xfrom][move.yfrom] = 0
            return
...
# --- add by me ---
    ...
    def remove(self, user_id):
        shutil.rmtree(os.getcwd() + '/' + user_id)
    ...
    def is_legal_notation(self, text):
        if len(text) == 3:
            if (text == 'o-o'):
                return ai.Move(ord(text[0]) - ord('a'), 8 - ord(text[1]) + ord('0'), -1, 0, True)
            else:
                return False
        elif len(text) == 4:
            ...
            if (text == 'o-o-o'):
                return ai.Move(ord(text[0]) - ord('a'), 8 - ord(text[1]) + ord('0'), -1, 1, True)
            else:
                return False
        elif len(text) == 6:
            if ord(text[0]) < ord('a') or ord(text[0]) > ord('h'):
            ...
            if text[5] == 'b' or text[5] == 'n' or text[5] == 'r' or text[5] == 'q':
                mov = ai.Move(ord(text[0]) - ord('a'), 8 - ord(text[1]) + ord('0'), ord(text[2]) - ord('a'), 8 - ord(text[3]) + ord('0'), False)
    ...
    def test_right_castle(self, color):
        if color == WHITE:
            ...
            self.chesspieces[4][7] != 0 and self.chesspieces[4][7].move == 0 and \
            self.chesspieces[5][7] == 0 and self.chesspieces[6][7] == 0:
                return 7
        elif color == BLACK:
            if self.chesspieces[7][0] != 0 and self.chesspieces[7][0].move == 0 and \
            self.chesspieces[4][0] != 0 and self.chesspieces[4][0].move == 0 and \
            self.chesspieces[5][0] == 0 and self.chesspieces[6][0] == 0:
                return 0
        return None

    def test_left_castle(self, color):
        if color == WHITE:
            if self.chesspieces[0][7] != 0 and self.chesspieces[0][7].move == 0 and \
            self.chesspieces[4][7] != 0 and self.chesspieces[4][7].move == 0 and \
            self.chesspieces[1][7] == 0 and self.chesspieces[2][7] == 0 and self.chesspieces[3][7] == 0:
                return 7
        elif color == BLACK:
            if self.chesspieces[0][0] != 0 and self.chesspieces[0][0].move == 0 and \
            self.chesspieces[4][0] != 0 and self.chesspieces[4][0].move == 0 and \
            self.chesspieces[1][0] == 0 and self.chesspieces[2][0] == 0 and self.chesspieces[3][0] == 0:
                return 0
        return None
    ...
    def is_win(self, color):
        ...
        for x in range(8):
            for y in range(8):
                piece = self.chesspieces[x][y]
                if piece != 0 and piece.color == opcolor and piece.piece_type == KING:
                    return False
        return True
